...
function findPhpstanScripts(scripts) {
    ...
    for (const [name, raw] of Object.entries(scripts)) {
        ...
        const invokesPhpstan = commands.some((cmd) => {
            ...
            return (
                cmd.includes('phpstan') || cmd.includes('vendor/bin/phpstan')
            );
        });
...
function suggestCommand(phpstanScripts, fallbackInfo) {
    ...
    if (preferred) {
        return {
            ...
            rationale:
                "Uses the repo's Composer script (preferred for consistent config).",
        };
    }
    ...
    if (!fallbackInfo.binaryRelPath) {
        return {
            command: null,
            rationale:
                'No PHPStan binary detected under vendor/bin and no Composer script found.',
        };
    }

    const configArg = fallbackInfo.configRelPath
        ? ` -c ${fallbackInfo.configRelPath}`
        : '';

    return {
        command: `${fallbackInfo.binaryRelPath} analyse${configArg}`,
        rationale:
            'Falls back to vendor/bin/phpstan with an explicit config when needed.',
    };
}
...
function buildReport() {
    ...
    const phpstanConfigFiles = ['phpstan.neon', 'phpstan.neon.dist'].filter(
        (f) => isFile(path.join(repoRoot, f)),
    );
    const phpstanBaselineFiles = [
        'phpstan-baseline.neon',
        'phpstan-baseline.neon.dist',
    ].filter((f) => isFile(path.join(repoRoot, f)));

    let configRelPath = null;
    if (phpstanConfigFiles.includes('phpstan.neon'))
        configRelPath = 'phpstan.neon';
    else if (phpstanConfigFiles.includes('phpstan.neon.dist'))
        configRelPath = 'phpstan.neon.dist';

    const configAbsPath = configRelPath
        ? path.join(repoRoot, configRelPath)
        : null;
    const configText = configAbsPath ? readTextSafe(configAbsPath) : null;

    const binaryRelPath = isFile(
        path.join(repoRoot, 'vendor', 'bin', 'phpstan'),
    )
        ? 'vendor/bin/phpstan'
        : null;
    ...
    const composerScripts =
        composer?.scripts && typeof composer.scripts === 'object'
            ? composer.scripts
            : null;
    const phpstanScripts = composerScripts
        ? findPhpstanScripts(composerScripts)
        : [];
    ...
    const referencedDependencies = configText
        ? extractStubPackageReferences(configText)
        : [];
    ...
    if (!composer)
        notes.push(
            'No composer.json found; PHPStan is usually installed via Composer.',
        );
    if (phpstanConfigFiles.length === 0)
        notes.push('No phpstan.neon or phpstan.neon.dist found at repo root.');
    if (!binaryRelPath && phpstanScripts.length === 0)
        notes.push(
            'No PHPStan entrypoint detected (Composer script or vendor/bin/phpstan).',
        );
