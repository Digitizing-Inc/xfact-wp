...
function main() {
    ...
    if (!info.ok) {
        report.notes.push(
            'WP-CLI not available on PATH. Run in the intended environment (container/ssh) or install WP-CLI.',
        );
    ...
    canRun(
        report,
        isInstalled,
        'WordPress not detected at the given --path/--url (check wp-config.php and targeting).',
    );
    if (!isInstalled.ok) {
        process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
        return;
    ...
    const doctorHelp = runWp(['doctor', '--help'], {
        pathArg: opts.path,
        urlArg: opts.url,
        allowRoot: opts.allowRoot,
    });
    ...
    const profileHelp = runWp(['profile', '--help'], {
        pathArg: opts.path,
        urlArg: opts.url,
        allowRoot: opts.allowRoot,
    });
    ...
    if (opts.path) {
        ...
        report.perfSignals.hasObjectCacheDropin = existsFile(
            path.join(wpContent, 'object-cache.php'),
        );
        report.perfSignals.hasAdvancedCacheDropin = existsFile(
            path.join(wpContent, 'advanced-cache.php'),
        );
        report.perfSignals.hasQueryMonitorPlugin = existsFile(
            path.join(
                wpContent,
                'plugins',
                'query-monitor',
        ...
        report.perfSignals.hasPerformanceLabPlugin = existsFile(
            path.join(wpContent, 'plugins', 'performance-lab', 'load.php'),
        );
    }

    if (!report.commands.doctor.available)
        report.notes.push(
            'Tip: install WP-CLI doctor: `wp package install wp-cli/doctor-command`.',
        );
    if (!report.commands.profile.available)
        report.notes.push(
            'Tip: install WP-CLI profile: `wp package install wp-cli/profile-command`.',
        );
